	...
	printf("===============================\n");
		...
		do
		{
			...
			if (pemain[i%2]==1)
				...
				tanda = '0';
			printf("a/               ************** GILIRAN PEMAIN %d ************** \n", pemain[i%2]);
			printf("n\t\t\tPilih Tempat : ");
			scanf("%d", &pilih);
			fflush(stdin);
			...
			default :
				...
				printf("Pilihan anda tidak sesuai, silahkan ulangi lagi!\n\n");
				i--;
		...
		while (hasil ==-1);
			tampil(matrix);
		if (hasil ==1)
			printf("\t ==>t\t\a\aPEMAIN %d MENANG !!!\n", pemain[(i+1)%2]);
		else 
			printf("\t ==>t\t\a\aPEMAIN %d SERI !!!\n");
		getche();
		}
